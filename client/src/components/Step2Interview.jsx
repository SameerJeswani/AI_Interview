...
const submitAnswer = async () => {
  ...
  try {
    ...
    console.log("3 After API");

    setFeedback(result.data.feedback);

    console.log("4 Before speakText");

    await speakText(result.data.feedback);

    console.log("5 After speakText");

  } catch (error) {
    console.error("SUBMIT ERROR:", error);
  } finally {
    console.log("6 Finally Executed");
...
setIsMicOn(false);
  try {
    const result = await axios.post(
      ServerUrl + "/api/interview/finish",{interviewId}, { withCredentials: true }
    );
